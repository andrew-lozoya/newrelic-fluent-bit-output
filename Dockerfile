...
COPY --from=builder /go/src/github.com/newrelic/newrelic-fluent-bit-output/out_newrelic_win64.dll /fluent-bit/bin/
COPY *.conf /fluent-bit/etc/

CMD ["cmd", "/c", "/fluent-bit/bin/fluent-bit.exe", "-c", "/fluent-bit/etc/fluent-bit.conf", "-e", "/fluent-bit/bin/out_newrelic_win64.dll" ]
